...
class TestGetUserActivityGroupInstancesMetricTracker:
    @pytest.fixture
    def setup_data(self):
    ...
    def test_user_activity_group_instance_metric_tracker_dtos(
            self, setup_data):
        user_activity_group_instance_ids, expected_response = setup_data
        storage = ActivityGroupStorageImplementation()

        response = storage.get_user_activity_group_instances_metric_tracker(
            user_activity_group_instance_ids,
        )
