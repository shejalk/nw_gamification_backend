"""
# Test Case for when user has exhausted montlhy streak freeze limit
"""
import freezegun
import pytest
import json
...
from nw_activities.constants.config import MAXIMUM_FREEZES_ALLOWED_PER_MONTH
...
from nw_activities.constants.enum import FrequencyTypeEnum, \
    ...
    InstanceTypeEnum, ResourceNameEnum
from nw_activities.tests.common_fixtures.adapters import \
    get_user_resource_mock, update_user_resources_mock


@freezegun.freeze_time("2022-07-15 10:00:00")
class TestCase02CreateStreakFreezeV1APITestCase(TestUtils):
    APP_NAME = APP_NAME
    ...
    def setup_data(self, mocker):
        ...
        for each in range(MAXIMUM_FREEZES_ALLOWED_PER_MONTH):
            UserActivityGroupInstanceFactory(
                user_id=user_id,
                activity_group_id=activity_group_ids[0],
                completion_percentage=0.0,
                instance_identifier=f"2022-07-2{each} 07:00:00#2022-07-"
                                    f"2{each} "
                                    f"23:00:00",
                completion_status=CompletionStatusEnum.YET_TO_START.value,
                instance_type=InstanceTypeEnum.FREEZE.value
